...
extern "C" __global__ void computeFixedField(
        ...
        const uint4* __restrict__ covalentFlags, const int2* __restrict__ exclusionTiles,
    ...
    while (pos < end) {
        ...
        if (includeTile) {
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    real3 fields[2];
                    computeOneInteraction(data, localData[tbx+tj], delta, (float) PSCALE_DEFAULT, fields);
                    data.field += fields[0];
                    localData[tbx+tj].field += fields[1];
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
