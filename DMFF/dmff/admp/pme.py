    ...
    def generate_get_energy(self):
        ...
        if not self.lpol:
            ...
            def get_energy(
                ...
                # JAX 0.7 disallows writing traced values back to object state from
                # inside jit-transformed functions. The MD runtime only needs the
                # optimized dipoles for the current energy evaluation, so skip the
                # cache update while tracing.
                if not isinstance(U_ind, jax.core.Tracer):
                    self.U_ind = U_ind
    ...
    def optimize_Uind(
        ...
        if steps_pol is None:
            for i in range(maxiter):
                ...
                masked_field = jnp.where(site_filter[:, jnp.newaxis], jnp.abs(field), 0.0)
                if jnp.max(masked_field) < thresh:
                    break
...
def calc_e_ind(dr, thole1, thole2, dmp, pscales, dscales, kappa, lmax=2):
    ...
    # Gaussian damping with the original MPID/DMFF choice of the width parameter:
    # pscale == 0 ? thole1 + thole2 : DEFAULT_THOLE_WIDTH
    w = jnp.heaviside(pscales, 0)
    a = w * DEFAULT_THOLE_WIDTH + (1 - w) * (thole1 + thole2)
    au = trim_val_infty(a * u)
    au2 = trim_val_infty(au * au)
    expau2 = jnp.piecewise(
        au2, [au2 < 50, au2 >= 50], [lambda x: jnp.exp(-x), lambda x: jnp.array(0)]
    )
    au4 = trim_val_infty(au2 * au2)
    au6 = trim_val_infty(au4 * au2)
    au8 = trim_val_infty(au4 * au4)
    au10 = trim_val_infty(au8 * au2)
    au12 = trim_val_infty(au10 * au2)

    # Gaussian damping factors for energies and radial derivatives.
    thole_c = 1.0 - expau2 * (1.0 + au2)
    thole_d0 = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4)
    thole_d1 = 1.0 - expau2 * (1.0 + au2)
    thole_q0 = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4 + au6 / 6.0 + au8 / 18.0)
    thole_q1 = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4 + au6 / 6.0)
    dthole_c = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4)
    dthole_d0 = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4 + au6 / 6.0 + au8 / 12.0)
    dthole_d1 = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4 + au6 / 6.0)
    dthole_q0 = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4 + au6 / 6.0 + au8 / 24.0 + au10 / 72.0)
    dthole_q1 = 1.0 - expau2 * (1.0 + au2 + 0.5 * au4 + au6 / 6.0 + au8 / 24.0)
    ...
    if lmax >= 3:
        ...
        udo_m0 = (
            -8.0 * 
            rInvVec[5] * 
            (pscales * thole_q0 + bVec[4] + (2/15)*alphaRVec[7]*X)
        )
        udo_m1 = (
            2.0 * 
            jnp.sqrt(6) * 
            (pscales * thole_q1 + bVec[4]) * 
            rInvVec[5]
        )
    else:
        udo_m0 = 0.0
