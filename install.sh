#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
#  install.sh — Build & install the PhyNEOForcePlugin
#
#  Prerequisites:
...
echo "============================================================"
echo "  PhyNEOForcePlugin Installer"
...
if conda env list | grep -qw "^${ENV_NAME} "; then
    ...
    conda create -y -n "${ENV_NAME}" -c conda-forge \
        python=3.11 openmm">=8.4" swig cmake make cxx-compiler numpy pip
    conda activate "${ENV_NAME}"
fi

# Ensure core build dependencies are present
conda install -y -c conda-forge python=3.11 openmm">=8.4" swig cmake make cxx-compiler numpy pip 2>/dev/null || true
...
print(f'  phyneoforceplugin: OK')
print(f'  mpidplugin shim:   OK')
...
echo "  Test:    cd examples/li_dmc_compare && python check_reference_install.py"
echo "============================================================"
