...
__device__ void computeOneInteraction(AtomData& atom1, AtomData& atom2, bool hasExclusions, float pScale, float dScale, float mScale, float forceFactor, mixed& energy) {
    ...
    // Uind-Uind terms (m=0)
    real eCoef = -8*rInvVec[3]*dScale*thole_d0;
    real dCoef = 12*rInvVec[4]*dScale*dthole_d0;
    ...
    // Uind-Uind terms (m=1)
    eCoef = 4*rInvVec[3]*dScale*thole_d1;
    dCoef = -6*rInvVec[4]*dScale*dthole_d1;
        ...
        if (x == y) {
            ...
            for (unsigned int j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 != atom2 && atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    ...
                    computeOneInteraction(data, localData[tbx+j], true, p, d, m, 0.5f, energy);
                }
            }
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    ...
                    computeOneInteraction(data, localData[tbx+tj], true, p, d, m, 1, energy);
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
            data.force *= -ENERGY_SCALE_FACTOR;
    ...
    while (pos < end) {
        ...
        if (includeTile) {
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    computeOneInteraction(data, localData[tbx+tj], false, (float) PSCALE_DEFAULT, (float) DSCALE_DEFAULT, (float) MSCALE_DEFAULT, 1, energy);
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
            data.force *= -ENERGY_SCALE_FACTOR;
