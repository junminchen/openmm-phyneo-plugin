...
void CudaCalcMPIDForceKernel::initialize(const System& system, const MPIDForce& force) {
    ...
    if (fabs(mScales[0]-mScales[1]) > 1e-12) {
        throw OpenMMException("MPID CUDA kernel currently requires mScale12 == mScale13.");
    }
    if (fabs(pScales[0]-pScales[1]) > 1e-12) {
        throw OpenMMException("MPID CUDA kernel currently requires pScale12 == pScale13.");
    }
    for (int i = 0; i < 5; i++) {
        if (fabs(dScales[i]-pScales[i]) > 1e-12) {
            throw OpenMMException("MPID CUDA kernel currently requires dScales == pScales for all shells.");
        }
    }
    ...
    defines["PSCALE12_13"] = cu.doubleToString(pScales[0]);
