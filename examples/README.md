...
`ec_liquid_216.pdb` and `ec_forcefield.xml` files.  The default PDB is a
216-molecule Packmol box at 1.321 g/mL, generated by
`examples/ec_density/prepare_pdb.py`.  The script adds the EC topology bonds
required by the XML template, chooses CUDA/CPU/Reference through `--platform`,
reports NPT density during the run, and prints final energy and density.

```bash
python examples/ec_density/prepare_pdb.py
