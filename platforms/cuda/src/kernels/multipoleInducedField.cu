...
#ifdef USE_EWALD
__device__ void computeOneInteraction(AtomData& atom1, AtomData& atom2, real3 deltaR, float pScale, float dScale, bool isSelfInteraction) {
    ...
    if (r2 < CUTOFF_SQUARED) {
        ...
        real scale3 = 1 - expdamp*(1 + damp + damp*damp/2);
        real scale5 = 1 - expdamp*(1 + damp + damp*damp/2 + damp*damp*damp/6);
        real scale7 = 1 - expdamp*(1 + damp + damp*damp/2 + damp*damp*damp/6 + damp*damp*damp*damp/30);
...
__device__ void computeOneInteraction(AtomData& atom1, AtomData& atom2, real3 deltaR, float pScale, float dScale, bool isSelfInteraction) {
    ...
    rr3 *= dScale*(1 - dampExp*(1 + damp + damp*damp/2));
    rr5 *= dScale*(1 - dampExp*(1 + damp + damp*damp/2 + damp*damp*damp/6));
    rr7 *= dScale*(1 - dampExp*(1 + damp + damp*damp/2 + damp*damp*damp/6 + damp*damp*damp*damp/30));
...
extern "C" __global__ void computeInducedField(
        unsigned long long* __restrict__ field, const real4* __restrict__ posq, const uint4* __restrict__ covalentFlags, const int2* __restrict__ exclusionTiles, 
        ...
        if (x == y) {
            ...
            for (unsigned int j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS){
                    ...
                    computeOneInteraction(data, localData[tbx+j], delta, p, d, atom1 == atom2);
                }
            }
        }
        else {
            ...
            for (unsigned int j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS){
                    ...
                    computeOneInteraction(data, localData[tbx+tj], delta, p, d, false);
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
        }
    ...
    while (pos < end) {
        ...
        if (includeTile) {
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS)
                    computeOneInteraction(data, localData[tbx+tj], delta, (float) PSCALE_DEFAULT, (float) DSCALE_DEFAULT, false);
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
