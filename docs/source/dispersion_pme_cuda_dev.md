...
5. Force-field XML now supports an independent `<Dispersion .../>` child under
   `ADMPPmeForce`, parallel to `Multipole` and `Polarize`.
...
2. `TestSerializeADMPPmeForce` passes.
...
export LD_PRELOAD=/path/to/build-openmm840-linux-cuda/libPhyNEOForcePlugin.so
...
LD_LIBRARY_PATH=$PWD:/path/to/env/lib:$LD_LIBRARY_PATH ./serialization/tests/TestSerializeADMPPmeForce
LD_LIBRARY_PATH=$PWD:/path/to/env/lib:$LD_LIBRARY_PATH ./platforms/cuda/tests/TestCudaADMPPmeForce
```
