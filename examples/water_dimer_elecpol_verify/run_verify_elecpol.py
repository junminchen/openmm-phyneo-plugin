...
class MPIDCalculator:
    """Thin wrapper around an OpenMM context that keeps only ADMPPmeForce."""
    ...
    def __init__(self, pdb_path: str, ff_xml: str, use_pme: bool = True):
        ...
        # Remove everything except ADMPPmeForce.
        # SWIG wraps ADMPPmeForce as generic openmm.Force; identify via XML.
        forces_to_remove = []
        mpid_found = False
        for i in range(system.getNumForces()):
            xml_str = mm.XmlSerializer.serialize(system.getForce(i))
            if "ADMPPmeForce" in xml_str or "Multipole" in xml_str:
                mpid_found = True
            else:
                forces_to_remove.append(i)
        assert mpid_found, "ADMPPmeForce not found in system"
