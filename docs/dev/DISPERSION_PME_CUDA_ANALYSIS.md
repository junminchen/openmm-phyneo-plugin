...
## 1. HEADER FILE ANALYSIS: ADMPPmeCudaKernels.h

### Member Variables Related to Dispersion
**File:** `/home/am3-peichenzhong-group/Documents/project/PhyNEO/MPIDForceplugin_repo/platforms/cuda/src/ADMPPmeCudaKernels.h`
...
**File:** `platforms/cuda/src/ADMPPmeCudaKernels.h`
...
```cpp
void initialize(const System& system, const ADMPPmeForce& force);
```
...
## 2. IMPLEMENTATION FILE ANALYSIS: ADMPPmeCudaKernels.cpp
...
## 4. API LAYER: ADMPPmeForce (openmmapi)

### 4.1 Dispersion Parameters Data Structure

**DispersionInfo Class (ADMPPmeForce.h, Lines 669-673):**
```cpp
class ADMPPmeForce::DispersionInfo {
public:
...
### 4.2 Dispersion Storage in ADMPPmeForce

**Member Variables (ADMPPmeForce.h, Lines 609-613):**
```cpp
...
**File:** `openmmapi/src/ADMPPmeForce.cpp`
...
```cpp
void ADMPPmeForce::getDispersionParameters(int index, double& c6, double& c8, double& c10) const {  // Line 352
...
```cpp
void ADMPPmeForce::setUseDispersionPME(bool use) {  // Line 360
    useDispersionPme = use;                       // Line 361
}

bool ADMPPmeForce::getUseDispersionPME() const {    // Line 364
...
```cpp
void ADMPPmeForce::setDispersionPmax(int pmax) {                           // Line 368
    if (pmax != 6 && pmax != 8 && pmax != 10)
        throw OpenMMException("ADMPPmeForce: dispersionPmax must be 6, 8, or 10");  // Line 370
...
```cpp
void ADMPPmeForce::setDPMEParameters(double alpha, int dnx, int dny, int dnz) {  // Line 378
...
void ADMPPmeForce::getDPMEParameters(double& alpha, int& dnx, int& dny, int& dnz) const {  // Line 385
...
```
execute() [ADMPPmeCudaKernels.cpp, Line 62]
  |
  ├─ if (useDispersionPme) [Line 993]
  |   |
