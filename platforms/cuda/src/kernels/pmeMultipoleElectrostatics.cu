...
__device__ int computeCovalentShell(uint4 covalent, int index) {
    int mask = 1<<index;
    return ((covalent.x & mask) ? 1 : 0) | ((covalent.y & mask) ? 2 : 0) | ((covalent.z & mask) ? 4 : 0);
}

__device__ float computeMScaleFactor(uint4 covalent, int index) {
    switch (computeCovalentShell(covalent, index)) {
        case 1: return (float) MSCALE12;
        case 2: return (float) MSCALE13;
        case 3: return (float) MSCALE14;
        case 4: return (float) MSCALE15;
        case 5: return (float) MSCALE16;
        default: return (float) MSCALE_DEFAULT;
    }
}

__device__ float computePScaleFactor(uint4 covalent, int index) {
    switch (computeCovalentShell(covalent, index)) {
        case 1: return (float) PSCALE12;
        case 2: return (float) PSCALE13;
        case 3: return (float) PSCALE14;
        case 4: return (float) PSCALE15;
        case 5: return (float) PSCALE16;
        default: return (float) PSCALE_DEFAULT;
    }
}

__device__ float computeDScaleFactor(uint4 covalent, int index) {
    switch (computeCovalentShell(covalent, index)) {
        case 1: return (float) DSCALE12;
        case 2: return (float) DSCALE13;
        case 3: return (float) DSCALE14;
        case 4: return (float) DSCALE15;
        case 5: return (float) DSCALE16;
        default: return (float) DSCALE_DEFAULT;
    }
}
...
__device__ void computeOneInteraction(AtomData& atom1, AtomData& atom2, bool hasExclusions, float pScale, float dScale, float mScale, float forceFactor,
    ...
    // Uind-Uind terms (m=0)
    real eCoef = -2*fourThirds*rInvVec[3]*(3*(dScale*thole_d0 + bVec[3]) + alphaRVec[3]*X);
    real dCoef = 2*rInvVec[4]*(6*(dScale*dthole_d0 + bVec[3]) + 4*alphaRVec[5]*X);
    ...
    // Uind-Uind terms (m=1)
    eCoef = 4*rInvVec[3]*(dScale*thole_d1 + bVec[3] - twoThirds*alphaRVec[3]*X);
    dCoef = -6*rInvVec[4]*(dScale*dthole_d1 + bVec[3]);
...
extern "C" __global__ void computeElectrostatics(
        ...
        const real4* __restrict__ posq, const uint4* __restrict__ covalentFlags,
    ...
    for (int pos = firstExclusionTile; pos < lastExclusionTile; pos++) {
        ...
        uint4 covalent = covalentFlags[pos*TILE_SIZE+tgx];
        if (x == y) {
            ...
            for (unsigned int j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 != atom2 && atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    ...
                    float d = computeDScaleFactor(covalent, j);
                    computeOneInteraction(data, localData[tbx+j], true, p, d, m, 0.5f, energy, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
                }
            }
            if (atom1 < NUM_ATOMS)
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    ...
                    float d = computeDScaleFactor(covalent, tj);
                    computeOneInteraction(data, localData[tbx+tj], true, p, d, m, 1, energy, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
            data.force *= -ENERGY_SCALE_FACTOR;
    ...
    while (pos < end) {
        ...
        if (includeTile) {
            ...
            for (j = 0; j < TILE_SIZE; j++) {
                ...
                if (atom1 < NUM_ATOMS && atom2 < NUM_ATOMS) {
                    computeOneInteraction(data, localData[tbx+tj], false, (float) PSCALE_DEFAULT, (float) DSCALE_DEFAULT, (float) MSCALE_DEFAULT, 1, energy, periodicBoxSize, invPeriodicBoxSize, periodicBoxVecX, periodicBoxVecY, periodicBoxVecZ);
                }
                tj = (tj + 1) & (TILE_SIZE - 1);
            }
            data.force *= -ENERGY_SCALE_FACTOR;
