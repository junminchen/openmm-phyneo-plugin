...
class ADMPPmeForce : public Force {
...
public:
    ...
    enum CovalentType {
                          Covalent12 = 0, Covalent13 = 1, Covalent14 = 2, Covalent15 = 3, Covalent16 = 4,
                          PolarizationCovalent11 = 5, PolarizationCovalent12 = 6, PolarizationCovalent13 = 7, PolarizationCovalent14 = 8, CovalentEnd = 9 };

    /**
     * Create an ADMPPmeForce.
     */
    ADMPPmeForce();

    /*
     * Methods for casting a Force to an ADMPPmeForce.
    */
    %extend {
        static OpenMM::ADMPPmeForce& cast(OpenMM::Force& force) {
            return dynamic_cast<OpenMM::ADMPPmeForce&>(force);
        }

        static bool isinstance(OpenMM::Force& force) {
            return (dynamic_cast<OpenMM::ADMPPmeForce*>(&force) != NULL);
        }
    }
   ...
   /**
    ...
    bool usesPeriodicBoundaryConditions() const {
        return nonbondedMethod == ADMPPmeForce::PME;
    }
...
class MPIDGenerator(object):
    ...
    """A MPIDGenerator constructs a ADMPPmeForce."""
    ...
    def setAxisType(kIndices):
                ...
                axisType = ADMPPmeForce.ZThenX
                if (not kz):
                    axisType = ADMPPmeForce.NoAxisType
                if (kz and not kx):
                    axisType = ADMPPmeForce.ZOnly
                if (kz and kzNegative or kx and kxNegative):
                    axisType = ADMPPmeForce.Bisector
                if (kx and kxNegative and ky and kyNegative):
                    axisType = ADMPPmeForce.ZBisect
                if (kz and kzNegative and kx and kxNegative and ky and kyNegative):
                    axisType = ADMPPmeForce.ThreeFold
    ...
    def parseElement(element, forceField):
        ...
        if len(existing) == 0:
            ...
            def merge_or_check(attr_name, incoming):
                ...
                if any(abs(float(a)-float(b)) > 1e-12 for a, b in zip(current, incoming)):
                    raise ValueError(f'Found multiple ADMPPmeForce tags with different {attr_name} arguments')

            merge_or_check("mScales", mScales)
            merge_or_check("pScales", pScales)
            merge_or_check("dScales", dScales)

        # Dispersion PME attributes on <ADMPPmeForce>
        ...
        if dispMScales is not None:
            if generator.dispMScales is not None and generator.dispMScales != dispMScales:
                raise ValueError('Found multiple ADMPPmeForce tags with different dispMScales arguments')
            generator.dispMScales = dispMScales

        # set type map: [ kIndices, multipoles, AMOEBA/OpenMM axis type]
        # Support both <Multipole> (ADMPPmeForce) and <Atom> (ADMPPmeForce) children
        ...
        # Dispersion parameters from <Dispersion> sub-elements within <ADMPPmeForce>
    ...
    def createForce(self, sys, data, nonbondedMethod, nonbondedCutoff, args):
        ...
        methodMap = {forcefield.NoCutoff:ADMPPmeForce.NoCutoff,
                     forcefield.PME:ADMPPmeForce.PME,
                     forcefield.LJPME:ADMPPmeForce.PME}
        ...
        def finalize_scales(scales):
            if scales is None:
                return None
            if len(scales) == 5:
                return [float(x) for x in scales] + [1.0]
            if len(scales) == 6:
                return [float(x) for x in scales]
            raise ValueError(f"Expected 5 or 6 scale values, got {len(scales)}")

        mScales = finalize_scales(resolve_scales("m", self.mScales))
        pScales = finalize_scales(resolve_scales("p", self.pScales))
        dScales = finalize_scales(resolve_scales("d", self.dScales))
        ...
        # get 1-2, 1-3, 1-4, 1-5, 1-6 bonded sets
        ...
        def expand_bonded_shell(previous_shell_sets, excluded_shell_sets):
            expanded_sets = []
            for i in range(len(data.atoms)):
                expanded = set()
                for j in previous_shell_sets[i]:
                    expanded = expanded.union(bonded12ParticleSets[j])
                for excluded in excluded_shell_sets:
                    expanded = expanded - excluded[i]
                expanded.discard(i)
                expanded_sets.append(set(sorted(expanded)))
            return expanded_sets

        # 1-4

        bonded14ParticleSets = expand_bonded_shell(bonded13ParticleSets, [bonded12ParticleSets, bonded13ParticleSets])

        # 1-5

        bonded15ParticleSets = expand_bonded_shell(bonded14ParticleSets, [bonded12ParticleSets, bonded13ParticleSets, bonded14ParticleSets])

        # 1-6

        bonded16ParticleSets = expand_bonded_shell(bonded15ParticleSets, [bonded12ParticleSets, bonded13ParticleSets, bonded14ParticleSets, bonded15ParticleSets])
            ...
            if t in self.typeMap:
                ...
                if (hit != 0):
                    ...
                    if (atomIndex == newIndex):
                        ...
                        force.setCovalentMap(atomIndex, ADMPPmeForce.Covalent15, tuple(bonded15ParticleSets[atomIndex]))
                        force.setCovalentMap(atomIndex, ADMPPmeForce.Covalent16, tuple(bonded16ParticleSets[atomIndex]))
