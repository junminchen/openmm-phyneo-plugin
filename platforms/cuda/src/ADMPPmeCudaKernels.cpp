...
class CudaCalcADMPPmeForceKernel::ForceInfo : public CudaForceInfo {
public:
    ForceInfo(const ADMPPmeForce& force) : force(force) {
    }
    ...
    void getParticlesInGroup(int index, vector<int>& particles) {
        ...
        force.getCovalentMap(particle, ADMPPmeForce::CovalentType(type), particles);
    }
    bool areGroupsIdentical(int group1, int group2) {
        return ((group1%7) == (group2%7));
    }
private:
    const ADMPPmeForce& force;
};

CudaCalcADMPPmeForceKernel::CudaCalcADMPPmeForceKernel(std::string name, const Platform& platform, CudaContext& cu, const System& system) : 
        CalcADMPPmeForceKernel(name, platform), cu(cu), system(system), hasInitializedScaleFactors(false), hasInitializedFFT(false), multipolesAreValid(false), hasCreatedEvent(false),
...
CudaCalcADMPPmeForceKernel::~CudaCalcADMPPmeForceKernel() {
    ...
    if (labFramePolarizabilities != NULL)
        delete labFramePolarizabilities;
    ...
    if (extrapolatedDipoleField != NULL)
        delete extrapolatedDipoleField;
...
void CudaCalcADMPPmeForceKernel::initialize(const System& system, const ADMPPmeForce& force) {
    ...
    for (int i = 0; i < numMultipoles; i++) {
        ...
        // If the multipole is isotropic, we flag it with a negative damping factor (which should always be positive) so we
        // can easily identify in the pairwise loops when we should be adding torque terms for induced dipoles.
        if(polarity[0]==polarity[1] && polarity[0]==polarity[2])
            damping *= -1;
    ...
    if (polarizationType == ADMPPmeForce::Mutual) {
        ...
        CHECK_RESULT(cuEventCreate(&syncEvent, CU_EVENT_DISABLE_TIMING), "Error creating event for ADMPPmeForce");
        ...
        extrapolatedDipoleField = new CudaArray(cu, 3*numMultipoles*(numOrders-1), elementSize, "extrapolatedDipoleField");
    ...
    for (int i = 0; i < numMultipoles; i++) {
        ...
        set<int> allAtoms;
        allAtoms.insert(i);
        force.getCovalentMap(i, ADMPPmeForce::Covalent12, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        for (int atom : atoms)
            covalentFlagValues.push_back(make_int3(i, atom, 1));
        force.getCovalentMap(i, ADMPPmeForce::Covalent13, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        for (int atom : atoms)
            covalentFlagValues.push_back(make_int3(i, atom, 2));
        force.getCovalentMap(i, ADMPPmeForce::Covalent14, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        for (int atom : atoms)
            covalentFlagValues.push_back(make_int3(i, atom, 3));
        force.getCovalentMap(i, ADMPPmeForce::Covalent15, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        for (int atom : atoms)
            covalentFlagValues.push_back(make_int3(i, atom, 4));
        force.getCovalentMap(i, ADMPPmeForce::Covalent16, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        for (int atom : atoms)
            covalentFlagValues.push_back(make_int3(i, atom, 5));
        force.getCovalentMap(i, ADMPPmeForce::PolarizationCovalent11, atoms);
        allAtoms.insert(atoms.begin(), atoms.end());
        ...
        vector<int> atoms12;
        force.getCovalentMap(i, ADMPPmeForce::PolarizationCovalent12, atoms12);
        for (int atom : atoms)
    ...
    if (polarizationType != ADMPPmeForce::Direct) {
        inducedField = new CudaArray(cu, 3*paddedNumAtoms, sizeof(long long), "inducedField");
    }
    usePME = (force.getNonbondedMethod() == ADMPPmeForce::PME);
    useDispersionPme = false;
    dispersionSelfEnergy = 0.0;
    dispersionPmax = 10;
    alphaDispersionEwald = 0.0;
    ...
    if (polarizationType == ADMPPmeForce::Extrapolated)
    ...
    else if (polarizationType == ADMPPmeForce::Mutual)
    ...
    defines["MSCALE12"] = cu.doubleToString(mScales[0]);
    defines["MSCALE13"] = cu.doubleToString(mScales[1]);
    defines["MSCALE14"] = cu.doubleToString(mScales[2]);
    defines["MSCALE15"] = cu.doubleToString(mScales[3]);
    defines["MSCALE16"] = cu.doubleToString(mScales[4]);
    defines["MSCALE_DEFAULT"] = cu.doubleToString(mScales[5]);
    defines["PSCALE12"] = cu.doubleToString(pScales[0]);
    defines["PSCALE13"] = cu.doubleToString(pScales[1]);
    defines["PSCALE14"] = cu.doubleToString(pScales[2]);
    defines["PSCALE15"] = cu.doubleToString(pScales[3]);
    defines["PSCALE16"] = cu.doubleToString(pScales[4]);
    defines["PSCALE_DEFAULT"] = cu.doubleToString(pScales[5]);
    defines["DSCALE12"] = cu.doubleToString(dScales[0]);
    defines["DSCALE13"] = cu.doubleToString(dScales[1]);
    defines["DSCALE14"] = cu.doubleToString(dScales[2]);
    defines["DSCALE15"] = cu.doubleToString(dScales[3]);
    defines["DSCALE16"] = cu.doubleToString(dScales[4]);
    defines["DSCALE_DEFAULT"] = cu.doubleToString(dScales[5]);
    ...
    if (usePME) {
        ...
        pmePhi = new CudaArray(cu, 35*numMultipoles, elementSize, "pmePhi");
...
void CudaCalcADMPPmeForceKernel::initializeScaleFactors() {
    cu.setAsCurrent();
    ...
    covalentFlags = CudaArray::create<uint4>(cu, nb.getExclusions().getSize(), "covalentFlags");
    vector<uint4> covalentFlagsVec(nb.getExclusions().getSize(), make_uint4(0, 0, 0, 0));
    for (int3 values : covalentFlagValues) {
        ...
        if (x == y) {
            ...
            covalentFlagsVec[index+offset1].x |= f0<<offset2;
            covalentFlagsVec[index+offset1].y |= f1<<offset2;
            covalentFlagsVec[index+offset1].z |= f2<<offset2;
            covalentFlagsVec[index+offset2].x |= f0<<offset1;
            covalentFlagsVec[index+offset2].y |= f1<<offset1;
            covalentFlagsVec[index+offset2].z |= f2<<offset1;
        }
        else if (x > y) {
            int index = exclusionTileMap[make_pair(x, y)]*CudaContext::TileSize;
            covalentFlagsVec[index+offset1].x |= f0<<offset2;
...
double CudaCalcADMPPmeForceKernel::execute(ContextImpl& context, bool includeForces, bool includeEnergy) {
    cu.setAsCurrent();
    ...
    void* computeMomentsArgs[] = {&cu.getPosq().getDevicePointer(), &multipoleParticles->getDevicePointer(),
        ...
        &polarizability->getDevicePointer(), &labFramePolarizabilities->getDevicePointer()};
    ...
    if (pmeGrid == NULL) {
       ...
       cu.executeKernel(electrostaticsKernel, electrostaticsArgs, numForceThreadBlocks*electrostaticsThreads, electrostaticsThreads);
        ...
        void* pmeTransformMultipolesArgs[] = {&labFrameDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(), &labFrameOctopoles->getDevicePointer(),
            &fracDipoles->getDevicePointer(), &fracQuadrupoles->getDevicePointer(), &fracOctopoles->getDevicePointer(), recipBoxVectorPointer[0], recipBoxVectorPointer[1], recipBoxVectorPointer[2]};
        cu.executeKernel(pmeTransformMultipolesKernel, pmeTransformMultipolesArgs, cu.getNumAtoms());
        void* pmeSpreadFixedMultipolesArgs[] = {&cu.getPosq().getDevicePointer(), &fracDipoles->getDevicePointer(), &fracQuadrupoles->getDevicePointer(), &fracOctopoles->getDevicePointer(),
        ...
        void* pmeFixedPotentialArgs[] = {&pmeGrid->getDevicePointer(), &pmePhi->getDevicePointer(), &field->getDevicePointer(),
            &cu.getPosq().getDevicePointer(), &labFrameDipoles->getDevicePointer(),
        ...
        void* pmeFixedForceArgs[] = {&cu.getPosq().getDevicePointer(), &cu.getForce().getDevicePointer(), &torque->getDevicePointer(),
            &cu.getEnergyBuffer().getDevicePointer(), &labFrameDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(), &labFrameOctopoles->getDevicePointer(),
            &fracDipoles->getDevicePointer(), &fracQuadrupoles->getDevicePointer(), &fracOctopoles->getDevicePointer(), &pmePhi->getDevicePointer(), &pmeCphi->getDevicePointer(),
            recipBoxVectorPointer[0], recipBoxVectorPointer[1], recipBoxVectorPointer[2]};
        ...
        if (cu.getUseDoublePrecision())
            ...
            cufftExecC2C(fft, (float2*) pmeGrid->getDevicePointer(), (float2*) pmeGrid->getDevicePointer(), CUFFT_INVERSE);
        void* pmeInducedPotentialArgs[] = {&pmeGrid->getDevicePointer(), &pmePhidp->getDevicePointer(),
           &cu.getPosq().getDevicePointer(), cu.getPeriodicBoxVecXPointer(), cu.getPeriodicBoxVecYPointer(),
        ...
        void* pmeInducedForceArgs[] = {&cu.getPosq().getDevicePointer(), &cu.getForce().getDevicePointer(), &torque->getDevicePointer(),
            &cu.getEnergyBuffer().getDevicePointer(), &labFrameDipoles->getDevicePointer(), &labFrameQuadrupoles->getDevicePointer(), &labFrameOctopoles->getDevicePointer(),
            &fracDipoles->getDevicePointer(), &fracQuadrupoles->getDevicePointer(), &fracOctopoles->getDevicePointer(),
...
void CudaCalcADMPPmeForceKernel::computeInducedField(void** recipBoxVectorPointer) {
    cu.setAsCurrent();
    CudaNonbondedUtilities& nb = cu.getNonbondedUtilities();
    int startTileIndex = nb.getStartTileIndex();
    int numTileIndices = nb.getNumTiles();
    ...
    computeInducedFieldArgs.push_back(&covalentFlags->getDevicePointer());
...
bool CudaCalcADMPPmeForceKernel::iterateDipolesByDIIS(int iteration) {
    cu.setAsCurrent();
    ...
    void* recordDIISDipolesArgs[] = {&field->getDevicePointer(), &inducedField->getDevicePointer(),
        &inducedDipole->getDevicePointer(), &labFramePolarizabilities->getDevicePointer(), &inducedDipoleErrors->getDevicePointer(), &prevDipoles->getDevicePointer(),
        &prevErrors->getDevicePointer(), &iteration, &trueValue, &diisMatrix->getDevicePointer()};
    cu.executeKernel(recordDIISDipolesKernel, recordDIISDipolesArgs, cu.getNumThreadBlocks()*cu.ThreadBlockSize, cu.ThreadBlockSize, cu.ThreadBlockSize*elementSize);
    float* errors = (float*) cu.getPinnedBuffer();
    ...
    if (48.033324*sqrt(total1/cu.getNumAtoms()) < inducedEpsilon)
        return true;

    // Compute the dipoles.

    void* updateInducedFieldArgs[] = {&inducedDipole->getDevicePointer(), &prevDipoles->getDevicePointer(),
                    &diisCoefficients->getDevicePointer(), &numPrev};
    cu.executeKernel(updateInducedFieldKernel, updateInducedFieldArgs, 3*cu.getNumAtoms(), 256);
    return false;
}

void CudaCalcADMPPmeForceKernel::computeExtrapolatedDipoles(void** recipBoxVectorPointer) {
    cu.setAsCurrent();
    ...
    for (int order = 1; order < maxExtrapolationOrder; ++order) {
        computeInducedField(recipBoxVectorPointer);
        void* iterateArgs[] = {&order, &inducedDipole->getDevicePointer(), &extrapolatedDipole->getDevicePointer(),
            &inducedDipoleFieldGradient->getDevicePointer(),
            &inducedField->getDevicePointer(), &extrapolatedDipoleField->getDevicePointer(), &extrapolatedDipoleFieldGradient->getDevicePointer(),
            &labFramePolarizabilities->getDevicePointer()};
...
void CudaCalcADMPPmeForceKernel::ensureMultipolesValid(ContextImpl& context) {
    cu.setAsCurrent();
...
void CudaCalcADMPPmeForceKernel::getLabFramePermanentDipoles(ContextImpl& context, vector<Vec3>& dipoles) {
...
void CudaCalcADMPPmeForceKernel::getInducedDipoles(ContextImpl& context, vector<Vec3>& dipoles) {
    ensureMultipolesValid(context);
    int numParticles = cu.getNumAtoms();
    dipoles.resize(numParticles);
    const vector<int>& order = cu.getAtomIndex();
...
void CudaCalcADMPPmeForceKernel::getTotalDipoles(ContextImpl& context, vector<Vec3>& dipoles) {
    ensureMultipolesValid(context);
    int numParticles = cu.getNumAtoms();
    dipoles.resize(numParticles);
    const vector<int>& order = cu.getAtomIndex();
...
void CudaCalcADMPPmeForceKernel::getElectrostaticPotential(ContextImpl& context, const vector<Vec3>& inputGrid, vector<double>& outputElectrostaticPotential) {
    ensureMultipolesValid(context);
...
void CudaCalcADMPPmeForceKernel::computeSystemMultipoleMoments(ContextImpl& context, vector<double>& outputMultipoleMoments) {
...
void CudaCalcADMPPmeForceKernel::getSystemMultipoleMoments(ContextImpl& context, vector<double>& outputMultipoleMoments) {
    ensureMultipolesValid(context);
    if (cu.getUseDoublePrecision())
...
void CudaCalcADMPPmeForceKernel::copyParametersToContext(ContextImpl& context, const ADMPPmeForce& force) {
...
void CudaCalcADMPPmeForceKernel::getPMEParameters(double& alpha, int& nx, int& ny, int& nz) const {
    if (!usePME)
