...
<ForceField>
 ...
 <ADMPPmeForce coulomb14scale="0.833333" defaultTholeWidth="8.0">
...
handled by the `ADMPPmeForce`.  The `ADMPPmeForce` does not handle Lennard-Jones (LJ)
...
The section defining the `ADMPPmeForce` terms looks like this:
...
cutoffs, _etc._.  For `ADMPPmeForce`, the `defaultTholeWidth` and
